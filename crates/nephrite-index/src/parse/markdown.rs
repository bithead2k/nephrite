...
pub fn parse_markdown(content: &str) -> MarkdownFacts {
    ...
    if let Some(raw) = fm_raw {
        ...
        let (json, leaves, fm_tags) = parse_yaml(&raw);
...
/// Parse complete YAML frontmatter. Malformed documents retain the legacy
/// best-effort reader so one bad note cannot prevent the vault from opening.
fn parse_yaml(raw: &str) -> (String, Vec<PropertyLeaf>, Vec<String>) {
    let Ok(value) = serde_yaml::from_str::<serde_yaml::Value>(raw) else {
        return parse_simple_yaml(raw);
    };
    let json = yaml_value_to_json(&value);
    let mut leaves = Vec::new();
    flatten_json_properties("", &json, &mut leaves);
    let mut tags = Vec::new();
    if let Some(value) = json.get("tags") {
        collect_json_tags(value, &mut tags);
    }
    (
...
fn merge_yaml_defaults(
    ...
    for mapping in mappings {
        if let serde_json::Value::Object(defaults) =
            yaml_value_to_json(&serde_yaml::Value::Mapping(mapping.clone()))
        {
    ...
    match value {
        serde_json::Value::Object(values) => {
            for (key, value) in values {
                ...
                flatten_json_properties(&child, value, leaves);
            }
        }
        serde_json::Value::Array(values) => {
            for (index, value) in values.iter().enumerate() {
                flatten_json_properties(&format!("{path}[{index}]"), value, leaves);
            }
        }
        _ => {}
    }
...
fn push_json_property(leaves: &mut Vec<PropertyLeaf>, path: &str, value: &serde_json::Value) {
    let prop_key = path
        .rsplit(['.', '['])
        .next()
        .unwrap_or(path)
    ...
    let (value_type, value_text, value_num, value_bool) = match value {
        ...
        serde_json::Value::Array(_) => ("array", None, None, None),
        serde_json::Value::Object(_) => ("object", None, None, None),
    };
...
/// Legacy fallback for damaged frontmatter.
...
"#;
    ...
    #[test]
    fn parses_full_yaml_frontmatter_and_merge_aliases() {
        let markdown = r#"---
title: "Quoted: title"
summary: >-
  first line
...
"#;
        ...
        let json: serde_json::Value =
            serde_json::from_str(facts.frontmatter_json.as_deref().unwrap()).unwrap();
        assert_eq!(json["title"], "Quoted: title");
        assert_eq!(json["summary"], "first line second line");
        assert_eq!(json["project"]["owner"], "Kirk");
        assert_eq!(json["project"]["enabled"], true);
        assert_eq!(json["project"]["owners"][1]["name"], "Grace");
        assert!(facts.properties.iter().any(|property| {
            property.prop_path == "project.owners[0].name"
                && property.value_text.as_deref() == Some("Ada")
        }));
        assert!(facts.tags.iter().any(|tag| tag.tag == "deep/research"));
    }
